...
- Skill 行为评测 21/21 已接入 CI，覆盖路由、缺参、claims、reference 边界、隐私和产品跳过决定。
...
- 八字、飞星、八宅、黄历、姓名、解梦、测字、节律在向导内完成临时表单、纯浏览器 Engine、claims 与 typed presentation；
- 查询和表单值仅在当前 React 内存，不写 history/localStorage；
- 真太阳时仍转交八字工作区，不在向导内补造证据。

P4-01 MVP 已覆盖高频命理、日用、风水与解释工具；其余候选继续安全转交既有工作区，后续仅按实际用户需求抽取更多直接执行组件；不得复制 Runner 或绕过 Dashboard 纯引擎边界。
