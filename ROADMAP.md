...
- 凶星（二黑/三碧/五黄/七赤）显示化煞物品建议。
- 吉星（一白/四绿/六白/八白/九紫）显示适宜房间用途与事业提示。
- 九宫格每宫有方位用途标签（财位/文昌/桃花/病符/凶位）。
- 顶部显示当前元运标识与九星旺衰状态。
...
- **"大游年动态年度游走"不存在**：四家开源仓库均无八星按年份游走的算法；八宅本身是静态查表（命卦→固定方位吉凶）。所谓"年度动态"实为玄空飞星（已由 `core.js getFlyingStars` 覆盖）。本计划不做独立的"动态大游年"，转而做八宅+飞星合参。
- **流派说明**：八宅方位表存在多个并行标准。本项目采用"百度百科/教材常见版"（除乾卦外原表已正确）。非坎卦行各家不同属正常流派分歧，不强行统一。
- **项目缺少的能力**：宅卦+命宅相配、方位用途宜忌、个人吉凶方位速查、八宅飞星合参、化解建议、形煞化解。
...
- 乾卦方位表与 2000+ 命卦公式修复后，1990/2000/2024 等年份命卦与方位吉凶与 suangua 一致。
- 八宅工作区支持输入房屋坐向，显示命宅相配等级与引文。
- 个人吉凶方位速查卡片显示四吉四凶 + 用途宜忌。
- 四凶位显示化解物品与化解法。
- 八宅+飞星合参：个人生气位与年飞星财位重合时提示"双重旺财"。
- 八宅术语解释齐全，无"暂无该术语"。
- 不破坏现有八宅计算逻辑、`EightMansionsChart` 渲染与 71 个单元测试。
...
对照 horosa 功能描述，本项目已覆盖（避免重复建设）：

- **自然语言意图 + 多系统调度**：SKILL.md「三层路由系统」（问题类型→学科路由→融合深度）+ 场景 A-E 工作流已是意图识别层；跨系统交叉点（命理+风水、命理+风水+中医、风水+流年、风水+道家等）已在 SKILL.md 第三步定义。
- **参数引导**：RULES.md §10 输入完备性清单 + 全局生辰范围校验 + 咨询向导六类入口，已是软约束与向导式引导。工具级硬闸门（`agent_guidance`）属工程化层，见「三层架构演进」节，随 MCP 启动一起做。
- **本地历史记忆**：HistoryStore localStorage 30 条脱敏历史已集成首页与 history 工作区；咨询向导调用 `toReading()` 存历史。
...
**已落地**：
- ✅ 新增命名「联合分析」入口（`combo` 工作区 + `comboEngine.ts` + 8 个 MCP 工具），一键聚合多系统结果：
  - **年度综合运势** `combo_annual_fortune` = 八字大运 + 五运六气 + 奇门年盘 + 紫微流年/大限 + 命卦方位（四系统一致性，2026-07-14 接入紫微流年）
  - **月度运势** `combo_monthly_fortune` = 流月干支 + 五运六气客气步 + 节气调养 + 紫微流月（年-月两级运势切片，2026-07-14 新增）
  ...
  - **今日养生建议** `combo_daily_wellness` = 体质 + 24节气 + 子午流注时辰 + 太岁/飞星方位（命理+体质+时空养生闭环，2026-07-13 新增）
  - **综合择日** `combo_zeri` = 黄历宜忌 + 神煞 + 太岁三煞 + 命卦吉方 联合筛吉日（2026-07-14 新增）
- ✅ 每个联合模块输出：整合结论 + 各术数看法依据 + 看法一致/有出入对照（看法一致则结论较稳，有出入则权衡标注）。
...
- 至少 1 个联合分析模块（如「年度综合运势」）可一键生成，输出含整合结论 + 各系统依据 + 各术数看法对照。
- 报告渲染呈现明确四层结构，且旧 sections 数据兼容不丢。
- 专项解读模式能根据具体问题给时间窗口 + 注意事项，而非泛泛全盘。
- 不破坏现有 Adapter 契约、toReading 字段、306 项测试（visual 229 + mcp 77）与各增强计划已完成的功能。
