...
describe('analyzeCharStructure 汉字结构', () => {
  it('字=上下结构', () => {
...
describe('calcCezi 测字', () => {
  it('返回完整结果含笔画/数理/五行/结构', () => {
    const r = calcCezi({ char: '明' });
  ...
  it('未收录字走估算并标注', () => {
    const r = calcCezi({ char: '𡈽' }); // 生僻字
    expect(r.strokesEstimated).toBe(true);
  });

  it('字义五行可取', () => {
    const r = calcCezi({ char: '江' });
    // 江含氵，字义五行应为水
  ...
  it('传入生辰时计算八字用神补益', () => {
    const r = calcCezi({ char: '江', birth: { year: 1990, month: 6, day: 15, hour: 12, gender: '男' } });
  ...
  it('不传生辰时 baziComplement 为 null', () => {
    const r = calcCezi({ char: '明' });
    expect(r.baziComplement).toBeNull();
  });

  it('export_snapshot 含分段解读', () => {
    const r = calcCezi({ char: '明' });
...
describe('calcCeziEnveloped', () => {
  it('返回 ToolEnvelope ok=true', () => {
    const env = calcCeziEnveloped({ char: '明' });
  ...
  it('未收录字产生 warnings', () => {
    const env = calcCeziEnveloped({ char: '𡈽' });
    expect(env.warnings?.some((w) => w.includes('未收录'))).toBe(true);
  });

  it('aspect 字段透传', () => {
    const env = calcCeziEnveloped({ char: '明', aspect: '事业' });
