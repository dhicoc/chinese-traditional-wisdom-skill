...
describe('calcMarriageCombo', () => {
  it('返回 ok=true 的 ToolEnvelope', () => {
    const env = calcMarriageCombo({ personA, personB });
  ...
  it('日柱权重最大（chongHeTotalScore 受日柱 3x 影响）', () => {
    const env = calcMarriageCombo({ personA, personB });
    const r = env.data;
  ...
  it('提供姓名时返回姓名匹配度，不提供时为 null', () => {
    const envFull = calcMarriageCombo({ personA, personB });
    expect(envFull.data.nameMatch).not.toBeNull();
    expect(envFull.data.nameMatch).toBeGreaterThanOrEqual(0);

    const envNoName = calcMarriageCombo({
  ...
  it('export_snapshot 含 8 段 sections', () => {
    const env = calcMarriageCombo({ personA, personB });
    const r = env.data;
  ...
  it('scene=合伙 时 purpose 为开业', () => {
    const env = calcMarriageCombo({ personA, personB, scene: '合伙' });
    expect(env.data.scene).toBe('合伙');
    expect(env.data.comboName).toContain('合伙');
  });

  it('日柱天克地冲时 chongHeTotalScore 为负（构造相冲案例）', () => {
