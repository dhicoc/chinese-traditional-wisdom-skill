...
const TOPIC_KEYWORDS: Partial<Record<ModuleId, string[]>> = {
  bazi: ['八字', '四柱', '命盘', '五行', '十神', '命理', '运势', '事业', '财运', '婚姻', '出生', '生辰', '命局'],
  ziwei: ['紫微', '斗数', '十二宫', '星曜', '四化', '命宫', '身宫', '紫微斗数'],
  liuyao: ['六爻', '起卦', '占卜', '纳甲', '世应', '用神', '六亲', '六神', '铜钱', '断卦', '占事'],
  meihua: ['梅花', '易数', '体用', '互卦', '变卦', '时间起卦', '数字起卦'],
  fengshui: ['风水', '罗盘', '二十四山', '方位', '堪舆', '阳宅', '阴宅', '八卦方位'],
  feixing: ['飞星', '九宫', '流年', '玄空', '九运', '元运'],
  bazhai: ['八宅', '游年', '命卦', '宅卦', '东四命', '西四命', '生气', '延年', '天医'],
  yunqi: ['五运六气', '运气', '岁运', '司天', '在泉', '客气', '主气', '六气'],
  tizhi: ['体质', '九种体质', '阳虚', '阴虚', '气虚', '痰湿', '湿热', '血瘀', '气郁', '特禀', '平和'],
  almanac: ['黄历', '宜忌', '吉时', '凶时', '节气', '物候', '农历', '今日宜'],
  namewuxing: ['姓名', '起名', '改名', '笔画', '三才', '五行', '名字'],
  dream: ['梦', '解梦', '梦境', '意象', '做梦'],
  rhythm: ['节律', '时辰', '经络', '养生', '子午流注', '气血'],
  mermaid: ['知识图谱', '流程图', '五行生克', '关系图'],
  reader: ['古籍', '原文', '八宅明镜', '经典', '书'],
};

const QUESTION_INTENT: Array<{ pattern: RegExp; module: ModuleId; reason: string }> = [
  // 占断决策类优先（要不要/能不能 等强占断信号）
  { pattern: /(要不要|能不能|成不成|该不该|占一卦|起卦|占卜|决断|犹豫|选择)/, module: 'liuyao', reason: '占断决策 → 六爻占卜' },
  { pattern: /(事业|工作|升职|求职|职业|职场)/, module: 'bazi', reason: '事业相关 → 八字命盘' },
  { pattern: /(财运|求财|赚钱|破财|投资|理财)/, module: 'bazi', reason: '财运相关 → 八字命盘' },
  { pattern: /(婚姻|感情|恋爱|结婚|对象|桃花|合婚)/, module: 'bazi', reason: '婚姻感情 → 八字命盘' },
  { pattern: /(健康|疾病|身体|生病|调养|养生)/, module: 'tizhi', reason: '健康体质 → 体质辨识' },
  { pattern: /(风水|方位|房子|住宅|办公室|布局|坐向)/, module: 'fengshui', reason: '风水方位 → 风水罗盘' },
  { pattern: /(流年|今年|明年|运势|太岁)/, module: 'feixing', reason: '流年运势 → 流年飞星' },
  { pattern: /(体质|气虚|阳虚|阴虚|痰湿|湿热)/, module: 'tizhi', reason: '体质辨识 → 九种体质' },
  { pattern: /(黄历|宜忌|吉日|择日|今天宜)/, module: 'almanac', reason: '黄历宜忌 → 每日黄历' },
  { pattern: /(姓名|名字|起名|改名)/, module: 'namewuxing', reason: '姓名分析 → 姓名五行' },
  { pattern: /(梦|解梦|梦境)/, module: 'dream', reason: '解梦 → 梦境意象' },
  { pattern: /(古籍|原文|经典|书)/, module: 'reader', reason: '古籍阅读 → Split Reader' },
];
