  ...
  return (
    <article className="rounded-card border border-white/8 bg-black/24 p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-[10px] text-zinc-500">{hit.source} · {hit.field}</span>
      </div>
      <h4 className="mt-2 text-sm font-semibold text-zinc-100">{hit.title}</h4>
      <p className="mt-1 text-xs leading-5 text-zinc-400">{hit.summary}</p>
  ...
  return (
    <section className="rounded-panel border border-ink-700 bg-ink-850/78 p-4 shadow-instrument">
          ...
          <button
            ...
            className={[
              ...
              item === term
                ...
                : 'border-white/8 bg-white/[0.03] text-zinc-500 hover:border-white/18 hover:text-zinc-200',
            ].join(' ')}
          >
            {item}
          </button>
      ...
      <label className="mt-3 flex items-center gap-2 rounded-card border border-white/8 bg-black/24 px-3 py-2">
        <span className="font-mono text-xs text-jade-500">引用</span>
        <input
          value={term}
