...
export interface BazhaiClaimViolation {
  index: number;
  kind: BazhaiPresentationClaim['kind'];
  message: string;
  ...
  claims.forEach((claim, index) => {
    if (claim.kind === 'mingGua') {
      ...
      if (claim.value !== expected) {
        violations.push({ index, kind: claim.kind, message: `命卦${claim.field} 与本次推算结果不一致。`, expected, actual: claim.value });
      }
      return;
    }
    ...
    if (claim.kind === 'direction') {
      ...
      if (claim.value !== expected) {
        violations.push({ index, kind: claim.kind, message: `${claim.direction}方的${claim.field === 'star' ? '游年星' : '吉凶'}与本次推算结果不一致。`, expected, actual: claim.value });
      }
      return;
    }
    ...
    if (claim.value !== expected) {
      violations.push({ index, kind: claim.kind, message: `${claim.field} 与本次目标年份方位结果不一致。`, expected, actual: claim.value });
    }
  });
