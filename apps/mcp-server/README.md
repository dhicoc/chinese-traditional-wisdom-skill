...
14. 未被专用呈现校验器覆盖的数值事实，使用本次成功计算结果的 `result_meta.numericAssertionToken` 调 `validate_numeric_assertions`。每条 claim 只能引用有限的 `data.*` 数值；`valid: true` 只表示结构化 claims 已核验，不能表示自由文本已自动校验。
