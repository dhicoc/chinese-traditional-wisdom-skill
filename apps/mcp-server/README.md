...
共 **34 个工具**：**32 个计算工具**与 2 个元工具（`agent_guidance` 参数引导、`wisdom_dispatch` 自然语言意图路由）。
...
- 元工具：`agent_guidance`、`wisdom_dispatch`
...
配置后重启客户端，应能看到 `chinese-wisdom` server 已连接并显示 34 个工具。
