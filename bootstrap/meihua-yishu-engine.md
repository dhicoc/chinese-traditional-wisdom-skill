...
实现：`apps/visual/src/legacy/meihuaEngine.ts`。支持时间、数字和揲蓍法；可用精确历法时标识 `local-exact`，否则标识 `local-approx`。
...
- 使用本地 `validateDivinationClaims(data, claims)` 核验；失败则删除断言或重新提取。
