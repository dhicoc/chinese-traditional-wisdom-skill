...
| 本地精确计算 | 内置 `lunar-javascript`，默认使用精确节气干支和大寒边界；用户可关闭后回退近似 | 八字、五运六气 |
| 本地规则计算 | 内置确定性规则或映射表，支持离线运行 | 梅花易数、风水罗盘、流年飞星、八宅 |
| 本地近似计算 | 精确历法关闭或加载失败时的离线 fallback | 八字、五运六气 |
| 演示数据 | 用于可视化结构展示，不代表真实排盘 | 紫微、六爻 |
| 需外部引擎 | 真实排盘需要接入外部库或人工规则 | 紫微、六爻精算 |
...
│
├── visual/                     # 可视化系统（纯前端，旧主入口）
│   ├── index.html              # 11 标签页 Dashboard
│   ├── test-runner.html        # 测试运行器
│   ├── js/engines/             # 纯 JS 排盘引擎（旧，留 fallback）
│   ├── js/capabilities.js      # 能力标识、报告导出、诊断入口
│   ├── js/engine-adapters.js   # 八字/五运六气/紫微/六爻/梅花统一 Adapter（旧）
│   ├── vendor/                 # 内置第三方浏览器库（lunar-javascript/iztro/3meta）
│   └── js/tests/               # 自动化测试与 schema 校验
│
├── apps/                       # 新架构（React Shell + MCP Server）
│   ├── visual/                 # React + Vite + TS Dashboard（SVG 可视化）
│   │   └── src/legacy/         # 纯 TS 引擎 + 10 个 ToolEnvelope 适配器
│   └── mcp-server/             # MCP Server（三层架构 Layer 2，10 工具薄壳）
│       ├── src/index.ts        # McpServer + StdioServerTransport 入口
│       ├── src/tools.ts        # 10 个工具定义（zod schema）
