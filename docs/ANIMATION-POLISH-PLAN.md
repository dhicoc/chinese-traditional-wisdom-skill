...
> 起草日期：2026-07-17。状态：**步骤 1-8 + 首页错峰完成**（7 跳过、9 跳过）。按 §步骤逐步落地，每步验证后提交。

## 额外完成（计划外）

- **首页卡片错峰入场**：HomeDashboard 两处卡片网格加 ct-animate-slide-up + ct-delay，依次淡入。
- **WorkspaceTabs Apple 风格滚动**：rAF + easeInOutCubic 缓动、选中居中、箭头微缩放、遮罩淡入淡出。
- **全仓数字输入框清空 bug 修复**：BirthPanel/Combo/Fengshui/Constitution 统一 draft 方案。

## 步骤 9（SVG 描边动画）跳过决定

评估后跳过：15 个命盘组件结构各异（rect 卡片块 vs path 线条圆图），描边动画在卡片块上效果差、在线条图上效果好，逐个适配工作量大且效果不统一。统一的"入场淡入"虽可覆盖但需逐个 Workspace 包装，回报不及首页错峰等已做项。留作未来按需单点优化。
