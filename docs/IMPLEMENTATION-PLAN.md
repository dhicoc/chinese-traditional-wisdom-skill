...
> 状态：P0-P3 与 Skill 行为评测已收口；P0-03、P1-02、P3-04 按产品决定跳过
> 基线日期：2026-08-19
> 基线分支：`main`
...
> 状态：✅ 已完成（2026-08-21）。字义数据拆为 32 个约 46–56KB 源分片；古籍全文索引均衡为 4 个约 184–224KB 分片；12MB 解梦公共单文件迁为 source 并生成 22 个不超过 700KB 的分类分片。生产预算要求所有 JS gzip ≤250KB、非 vendor raw ≤700KB、首屏入口 gzip ≤50KB，当前最大 gzip 217456 bytes，bundle budget 255/255 通过。全文索引仅 95 段且异步/防抖加载，当前不引入 Worker，避免额外线程与重复索引内存。
...
> 状态：⏭️ 不实施（2026-08-21，产品决定）。本项目定位为实用工具，不新增 `riskSafetyGate`、独立安全检查 CLI 或阻断工具执行的额外限制；保留现有全局 `RULES.md`、免责声明与 P0-02 已有路由提示，不扩大安全门范围。
...
> 状态：✅ 已完成（2026-08-21）。已建立 `skill-evals/cases`、`expected`、`run-evals.ts`，19/19 行为契约通过并接入 GitHub Actions；Runner 不调用模型或远端服务，报告不输出合成 query。
...
16. 不重新引入 `riskSafetyGate`；
17. 不持久化完整输入或原始问题；
18. P0-03 细粒度安全文案按决定跳过；
19. P1-02 能力状态拆分按决定跳过。
...
同名 case/expected 必须一一对应；任一断言失败退出码为 1 并阻断 CI。
...
P0、P1、P2、P3 与第 7 节 Skill 行为评测均已完成或按产品决定明确跳过。下一步进入维护与发布审计：依赖升级、规则变更或产品决定变化时先更新对应 eval，再运行完整质量门；不再新增术数功能。
