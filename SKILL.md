...
1. 检查 Python 依赖是否已安装：
   ...
   python -c "import iztro" 2>/dev/null && echo OK || echo MISSING
...
2. 如果任一缺失 → 执行安装脚本：
   ...
   安装脚本自动：pip install -r requirements.txt + npm install

3. 验证安装结果并向用户报告：
   ✅ Python: lunar-python ✓ / iztro-py ✓ / ichingshifa ✓
   ✅ Node.js: bazi-ziwei-skill ✓（如安装）
   ✅ Web Dashboard: visual/index.html ✓
   ✅ 自动化测试: visual/test-runner.html ✓

4. 确认就绪后，继续执行以下工作流。
   如果 pip install 失败 → 降级使用 visual/js/engines/ 纯前端引擎，告知用户部分功能受限
```
...
> 本项目提供 MCP server（`apps/mcp-server/`），把 10 个计算引擎暴露为 MCP 工具，供 Claude Code / Claude Desktop / Cursor / Cline 等客户端直接调用。**用户无需手动编辑配置文件**，AI 可自主配置激活。
...
**MCP 工具列表**（12 个）：bazi_calculate / ziwei_chart / cast_liuyao / arrange_qimen / cast_meihua / calc_yunqi / analyze_name / calc_xiyong / get_constitution_tendency / dream_interpret + agent_guidance（参数引导，防瞎猜）+ wisdom_dispatch（自然语言意图路由）。

> 三层架构：Layer 1 SKILL.md（本文件，路由）→ Layer 2 MCP Server（10+2 工具薄壳）→ Layer 3 Visual Dashboard（可选可视化）。MCP server 与 Dashboard 共享同一份纯 TS 引擎，互不依赖。
...
- 八字、五运六气：本地近似计算，可离线运行，但必须提示节气/历法简化。
- 风水罗盘、流年飞星、八宅：本地规则计算，依赖 JSON 映射表。
- 紫微：已接入本地 iztro v2.5.8 真实排盘，未加载时回退演示。
- 六爻：已接入本地京房八宫纳甲引擎(liuyao-engine.js)，输出真实纳甲/六亲/六神/世应/用神/变卦；引擎未加载时回退演示。
- 梅花：内置本地时间/数字起卦规则，不同流派口径可能有差异。
- 外部引擎：六爻等模块在命令行场景可选 ichingshifa 等外部引擎作交叉验证，Dashboard 已内置本地纳甲引擎优先。
