...
1. 确认计算引擎就绪（默认路径：纯 TS 引擎 + MCP 薄壳，零 Python 依赖）：
   - 所有 23 个计算引擎均为纯 TypeScript，位于 apps/visual/src/legacy/
...
3. 验证就绪并向用户报告：
   ...
   ✅ MCP Server: apps/mcp-server/ ✓（25 工具：23 计算 + agent_guidance + wisdom_dispatch）
...
> 本项目提供 MCP server（`apps/mcp-server/`），把 23 个计算引擎暴露为 MCP 工具，供 Claude Code / Claude Desktop / Cursor / Cline 等客户端直接调用。**用户无需手动编辑配置文件**，AI 可自主配置激活。
