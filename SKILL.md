...
1. 确认计算引擎就绪（默认路径：纯 TS 引擎 + MCP 薄壳，零 Python 依赖）：
   - 所有 24 个计算引擎均为纯 TypeScript，位于 apps/visual/src/legacy/
...
3. 验证就绪并向用户报告：
   ...
   ✅ MCP Server: apps/mcp-server/ ✓（26 工具：24 计算 + agent_guidance + wisdom_dispatch）
   ✅ Web Dashboard: apps/visual（React+SVG，pnpm dev 启动）✓
   ✅ 自动化测试: pnpm test（visual 单元 + e2e + mcp-server）✓

4. 确认就绪后，继续执行以下工作流。
   MCP 不可用时 → 仍可启动 React Dashboard（cd apps/visual && pnpm dev）用纯前端引擎，告知用户可视化可用、AI 直调工具需配 MCP
```
...
> 本项目提供 MCP server（`apps/mcp-server/`），把 24 个计算引擎暴露为 MCP 工具，供 Claude Code / Claude Desktop / Cursor / Cline 等客户端直接调用。**用户无需手动编辑配置文件**，AI 可自主配置激活。
...
**MCP 工具列表**（26 个 = 24 计算 + 2 元工具）：

- 排盘计算（15）：`bazi_calculate` / `ziwei_chart` / `cast_liuyao` / `arrange_qimen` / `liuren_calculate` / `xingxiu_daily` / `taiyi_calculate` / `huangji_calculate` / `cast_meihua` / `calc_yunqi` / `analyze_name` / `calc_xiyong` / `get_constitution_tendency` / `dream_interpret` / `cast_cezi`
