...
1. 确认计算引擎就绪（默认路径：纯 TS 引擎 + MCP 薄壳，零 Python 依赖）：
   - 所有 22 个计算引擎均为纯 TypeScript，位于 apps/visual/src/legacy/
...
3. 验证就绪并向用户报告：
   ✅ 纯 TS 引擎: apps/visual/src/legacy/*Engine.ts ✓（18 个，零 DOM 依赖）
   ✅ MCP Server: apps/mcp-server/ ✓（24 工具：22 计算 + agent_guidance + wisdom_dispatch）
   ✅ Web Dashboard: apps/visual（React+SVG）/ visual/index.html（旧入口）✓
   ✅ 自动化测试: pnpm test（visual 单元 + e2e + mcp-server）✓

4. 确认就绪后，继续执行以下工作流。
   MCP 不可用时 → 仍可双击 visual/index.html 用纯前端引擎，告知用户可视化可用、AI 直调工具需配 MCP
```
...
> 本项目提供 MCP server（`apps/mcp-server/`），把 22 个计算引擎暴露为 MCP 工具，供 Claude Code / Claude Desktop / Cursor / Cline 等客户端直接调用。**用户无需手动编辑配置文件**，AI 可自主配置激活。
...
> 三层架构：Layer 1 SKILL.md（本文件，路由）→ Layer 2 MCP Server（22+2 工具薄壳）→ Layer 3 Visual Dashboard（可选可视化）。MCP server 与 Dashboard 共享同一份纯 TS 引擎，互不依赖。
...
可通过 `FORTUNE.getCapabilities()` 查询能力状态，通过 `FORTUNE.exportReportData()` 生成脱敏报告快照。不要把演示数据表述为精确排盘。
...
入口文件：`apps/visual`（React + SVG Dashboard，主开发入口，`pnpm dev` 启动）或 `visual/index.html`（旧入口，双击可用）。多工作区一站式展示，纯前端无服务器依赖。

可视化数据生成逻辑见对应 bootstrap/ 指南的"结果可视化"段落。

**两种可视化模式**（详见上节）：
- **对话一次性 HTML 报告**：文字报告后附带静态 HTML 文件（Canvas 四柱+五行图）
- **Web Dashboard**：`apps/visual`（React + SVG）或 `visual/index.html`（旧入口）交互式多工作区面板（用户要求多次排盘时推荐）
