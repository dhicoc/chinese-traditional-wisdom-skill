...
1. 确认计算引擎就绪（默认路径：纯 TS 引擎 + MCP 薄壳，零 Python 依赖）：
   - 所有 18 个计算引擎均为纯 TypeScript，位于 apps/visual/src/legacy/
...
3. 验证就绪并向用户报告：
   ...
   ✅ MCP Server: apps/mcp-server/ ✓（20 工具：18 计算 + agent_guidance + wisdom_dispatch）
...
> 三层架构：Layer 1 SKILL.md（本文件，路由）→ Layer 2 MCP Server（18+2 工具薄壳）→ Layer 3 Visual Dashboard（可选可视化）。MCP server 与 Dashboard 共享同一份纯 TS 引擎，互不依赖。
