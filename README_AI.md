...
> **MCP 自动激活**（2026-07-10 架构重构后新增）：当用户在支持 MCP 的客户端（Claude Code/Desktop/Cursor/Cline）中想直接调用排盘工具时，AI 跑 `node scripts/setup-mcp.mjs` 一键自动检测并配置客户端，用户无需手动编辑 JSON。配置后 `chinese-wisdom` server 提供 25 个工具（23 计算 + agent_guidance 参数引导 + wisdom_dispatch 意图路由），统一返回 ToolEnvelope。详见 `apps/mcp-server/README.md`。
...
3. 对话中直接说「排个八字」「解梦」「今日养生」「合婚」等，AI 自动调用 23 个计算工具之一
...
| [apps/mcp-server/](apps/mcp-server/) | MCP Server（25 工具薄壳）+ `README.md` 挂载指南 |
