...
> **MCP 自动激活**（2026-07-10 架构重构后新增）：当用户在支持 MCP 的客户端（Claude Code/Desktop/Cursor/Cline）中想直接调用排盘工具时，AI 跑 `node scripts/setup-mcp.mjs` 一键自动检测并配置客户端，用户无需手动编辑 JSON。配置后 `chinese-wisdom` server 提供 37 个工具（32 计算 + `agent_guidance` 参数引导 + `validate_bazi_presentation` 八字呈现依据校验 + `validate_ziwei_presentation` 紫微呈现依据校验 + `validate_bazhai_presentation` 八宅呈现依据校验 + `wisdom_dispatch` 意图路由）。常规计算统一返回 ToolEnvelope；`resolve_true_solar_time` 返回可审计的校准结果。详见 `apps/mcp-server/README.md`。
