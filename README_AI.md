...
> **MCP 自动激活**（2026-07-10 架构重构后新增）：当用户在支持 MCP 的客户端（Claude Code/Desktop/Cursor/Cline）中想直接调用排盘工具时，AI 跑 `node scripts/setup-mcp.mjs` 一键自动检测并配置客户端，用户无需手动编辑 JSON。配置后 `chinese-wisdom` server 提供 12 个工具（10 计算 + agent_guidance 参数引导 + wisdom_dispatch 意图路由），统一返回 ToolEnvelope。详见 `apps/mcp-server/README.md`。
...
1. **纯 TS enveloped 引擎**（`apps/visual/src/legacy/*Engine.ts`，2026-07-10 架构重构后推荐路径）：零 DOM 依赖，统一返回 `ToolEnvelope`，MCP server 与 React Dashboard 共享。10 个引擎：bazi/ziwei/liuyao/qimen/meihua/yunqi/xiyong/nameRating/constitutionTendency/dream。
2. **纯 JS 本地引擎**（`visual/js/engines/`，旧路径，留 fallback）：双击 visual/index.html 即可运行，零依赖
3. **系统已安装引擎**：查 tool-index.md 确认本地可用
4. **引擎缺失**：按 bootstrap/ 目录下引导文件接入，询问用户是否安装
...
| [apps/mcp-server/](apps/mcp-server/) | MCP Server（三层架构 Layer 2，12 工具薄壳）+ `README.md` 挂载指南 |
