...
[1/2] 计算引擎（纯 TS，零 Python 依赖）:
  - apps/visual/src/legacy/*Engine.ts  19 个纯 TS 引擎
    （八字/紫微/六爻/奇门/大六壬/星宿/太乙/梅花/五运六气/姓名/喜用神/体质/解梦 + 6 联合分析）
  - 精确历法: lunar-javascript (npm ESM import)
  - 紫微: iztro v2.5.8 (npm ESM)    奇门: 3meta v2.6.0 (npm ESM)
  - 可视化: apps/visual (React + Vite + SVG)  /  visual/index.html (旧入口)
...
Quick start:
  ...
  # 或本地可视化:
  cd apps/visual && pnpm dev         # React + SVG Dashboard
  # 或双击旧入口:
  visual/index.html
...
1. 验证包结构完整性：
   ...
   - 必须存在：apps/visual（React Dashboard）或 visual/index.html（旧入口）
...
> **MCP 自动激活**（2026-07-10 架构重构后新增）：当用户在支持 MCP 的客户端（Claude Code/Desktop/Cursor/Cline）中想直接调用排盘工具时，AI 跑 `node scripts/setup-mcp.mjs` 一键自动检测并配置客户端，用户无需手动编辑 JSON。配置后 `chinese-wisdom` server 提供 21 个工具（19 计算 + agent_guidance 参数引导 + wisdom_dispatch 意图路由），统一返回 ToolEnvelope。详见 `apps/mcp-server/README.md`。
...
1. **纯 TS enveloped 引擎**（`apps/visual/src/legacy/*Engine.ts`，2026-07-10 架构重构后推荐路径）：零 DOM 依赖，统一返回 `ToolEnvelope`，MCP server 与 React Dashboard 共享。18 个引擎：bazi/ziwei/liuyao/qimen/liuren/xingxiu/taiyi/meihua/yunqi/analyzeName/xiyong/constitutionTendency/dream + 5 combo。
2. **纯 JS 本地引擎**（`visual/js/engines/`，旧路径，留 fallback）：双击 visual/index.html 即可运行，零依赖
3. **可选 Python oracle**（命令行交叉验证）：`ichingshifa`（六爻）/ `iztro-py`（紫微），默认不需要
4. **引擎缺失**：按 bootstrap/ 目录下引导文件接入，或走 MCP/纯 TS 路径
...
| 多系统聚合（`local-exact`） | 跨系统联合分析 + 一致性检验 | 联合分析（combo_*） |
...
3. 对话中直接说「排个八字」「解梦」「今日养生」等，AI 自动调用 19 个计算工具之一
...
| [visual/index.html](visual/index.html) | 交互式 Web Dashboard（旧入口，留 fallback） |
| [apps/mcp-server/](apps/mcp-server/) | MCP Server（三层架构 Layer 2，20 工具薄壳）+ `README.md` 挂载指南 |
| [scripts/setup-mcp.mjs](scripts/setup-mcp.mjs) | MCP 一键自动配置脚本（AI 自主激活） |

## 5. 全球搜索

索引三源数据：
- **CORE.termExplanations**：专业术语解释（定义在 visual/js/core.js，已扩充至 160+ 常用术语 0 缺失）
- **MAPPING_INDEX**：6 个 JSON 确定性映射表（knowledge-base/fengshui/mappings/）
- **KB_INDEX**：30 个古籍文件（knowledge-base/fengshui/）

搜索入口：React Dashboard 的 CommandBar，或旧入口 visual/index.html 右上角搜索按钮，或在 AI 对话中直接询问。
...
| 引擎缺失 | 降级方案 |
|---------|---------|
| MCP server 未配置 | 走纯 TS 引擎 + React Dashboard（双击 apps/visual 或 visual/index.html） |
