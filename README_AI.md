...
1. **纯 TS enveloped 引擎**（`apps/visual/src/legacy/*Engine.ts`，2026-07-10 架构重构后推荐路径）：零 DOM 依赖，统一返回 `ToolEnvelope`，MCP server 与 React Dashboard 共享。22 个引擎：bazi/ziwei/liuyao/qimen/liuren/xingxiu/taiyi/huangji/meihua/yunqi/analyzeName/xiyong/constitutionTendency/dream + 8 combo。
...
| [apps/mcp-server/](apps/mcp-server/) | MCP Server（三层架构 Layer 2，24 工具薄壳）+ `README.md` 挂载指南 |
