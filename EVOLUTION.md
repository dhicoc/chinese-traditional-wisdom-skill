...
### 2026-07-14 续：皇极经世引擎（邵雍元会运世宇宙周期 + 九卦配置）

补齐高端神数最后一块——皇极经世，与太乙同源同作者（kentang2017 系列）：
- 新增 `huangjiEngine.ts`（约 480 行）：参考 `kentang2017/kinwangji`（MIT）wanji.py 核心算法纯 TS 重写。
  - 元会运世周期：积年 67017+年 → 会（÷10800）/运（÷360）/世（÷30）。
  - 九卦配置：正卦（积年÷2160映射60卦循环主一运大势）→ 运卦（正卦变爻）→ 世卦（运卦变爻主一世30年）→ 旬卦（世卦变爻，动爻由年柱所在旬甲首定）→ 年卦/月卦/日卦/时卦/分卦（60甲子序与卦循环推算）。
  - 自建 64 卦数字码表（6位7/8，从下到上6爻）+ 皇极60卦循环 + 60甲子 + 六旬甲首。
  - 历法复用 lunar-javascript 真实干支农历（已实测验证精度：节气到秒、立春边界时刻级正确），无 solar 走公历近似回退。不引入 ephem/sxtwl 依赖。
- 新增 MCP `huangji_calculate` + guidance + dispatch 路由（皇极/元会运世/邵雍等关键词）。
- 新增 Dashboard `HuangjiWorkspace`：起盘信息（干支+积年+会运世+动爻）+ 九卦卡片（正/世/年卦高亮）+ 趋势解读 + 四层报告。
- 算法锚点验证：1990年积年69007÷2160=32→正卦鼎，与 kinwangji 一致。
- 工具总数：22 计算 + 2 元 = 24。visual 241 + mcp 80 = 321 测试全过。
